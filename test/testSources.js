...
export function array() { return Sequence.from([ 1, 2, 3 ]); }
...
export function of() { return Sequence.from(0, 1, 2, 3); }
export function ofObjects() { return Sequence.from({ prop1: 'asd', prop2: 50 }, { prop1: 'asd', prop2: 20 }, { prop1: 'kek', prop2: 10 }); }
export function filter() { return Sequence.from(0, 1, 2, 3, 4, 5).filter(k => k % 2 === 0); }
export function map() { return Sequence.from(0, 2, 4).map(k => k * 2); }
export function flatMap() { return Sequence.from([{ data: [ 1, 2, 3 ] }, { data: [ 4, 5, 6 ] }]).flatMap(k => k.data); }
...
export function distinctNumbers() { return Sequence.from(0, 2, 2, 0, 5, 10, 2, 5).distinct(); }
export function distinctObjects() { return Sequence.from('asd', 'sad', 'lal', 'a', 'c', 'ba').distinct(k => k.length); }
